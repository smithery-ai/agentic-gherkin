...
export function createClaudeEvaluator(options: ClaudeProviderOptions = {}): ScenarioEvaluator {
  return {
    name: "claude",
    evaluate: (request) => evaluateWithClaude(options, request, "scenario"),
    evaluateBatch: (request) => evaluateWithClaude(options, request, "batch"),
  };
}
  ...
  try {
    ...
    const stream = query({
      ...
      options: {
        ...
        permissionMode:
          (options.permissionMode ?? process.env.AGENTIC_GHERKIN_CLAUDE_PERMISSION_MODE ?? "dontAsk") as never,
        allowedTools: options.allowedTools,
        disallowedTools: options.disallowedTools,
        maxTurns: options.maxTurns ?? (mode === "batch" ? 20 : 12),
        ...
        stderr(data: string) {
          request.log.write({ provider: "claude", mode, stream: "stderr", data });
        },
      },
    });

    for await (const message of stream) {
      request.log.write({ provider: "claude", mode, message });
